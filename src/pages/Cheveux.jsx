...
export default function Cheveux() {
    ...
    const produitsCheveux = useMemo(() => {
        return produits.filter((produit) => produit.categorie === "Cheveux");
    }, [produits]);
    ...
    const { minPrix, maxPrix } = useMemo(() => {
        const allPrices = produitsCheveux.map(p => p.promo ? p.promoPrix : p.prix);
        return {
    ...
    const produitsFiltres = produitsCheveux.filter(p => {
    ...
    return (
        <div className="space-y-16">
            {/* 🔹 Hero Section */}
            <Hero image={'/images/c837a6_be9dec23651f456fa55221d745d14a9f~mv2.avif'} titre={"CHEVEUX"} />
