...
export default function Accessoires() {
    ...
    const produitsAccessoires = useMemo(() => {
        return produits.filter((produit) => produit.categorie === "Accessoires");
    }, []);
    ...
    const { minPrix, maxPrix } = useMemo(() => {
        const allPrices = produitsAccessoires.map(p => p.promo ? p.promoPrix : p.prix);
        return {
    ...
    const produitsFiltres = produitsAccessoires.filter(p => {
    ...
    return (
        <div className="space-y-16">
            {/* 🔹 Hero Section */}
            <Hero image={'/images/c837a6_ef1cecb54b7c405fa72066165766e46c~mv2.avif'} titre={"ACCESSOIRES"} />
