...
export default function Corps() {
    ...
    const produitsCorps = useMemo(() => {
        return produits.filter((produit) => produit.categorie === "Corps");
    }, []);
    ...
    const { minPrix, maxPrix } = useMemo(() => {
        const allPrices = produitsCorps.map(p => p.promo ? p.promoPrix : p.prix);
        return {
    ...
    const produitsFiltres = produitsCorps.filter(p => {
    ...
    return (
        <div className="space-y-16">
            {/* 🔹 Hero Section */}
            <Hero image={'/images/c837a6_adc8f0b0d2364271ab3d39214c485832~mv2.avif'} titre={"CORPS"} />
