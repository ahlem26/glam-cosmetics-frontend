...
const categories = [
    "Tout", "Visage", "Accessoires", "Parfums", "Cheveux", "Maquillage", "Corps", "Promos et offres", "Meilleures ventes"
];

export default function Nouveautes() {
    // 🔹 On filtre les produits récents ou marqués comme "nouveaux"...........
    const nouveautes = produits.filter(
        (produit) => produit.nouveau === true || new Date(produit.date) > new Date("2025-09-15")
    );

    // // 🔹 Date actuelle
    // const maintenant = new Date();

    // // 🔹 On filtre les produits ajoutés il y a moins d'un mois
    // const nouveautes = produits.filter((produit) => {
    //     const dateProduit = new Date(produit.date);
    ...
    const { minPrix, maxPrix } = useMemo(() => {
        const allPrices = nouveautes.map(p => p.promo ? p.promoPrix : p.prix);
        return {
    ...
    const produitsFiltres = nouveautes.filter(p => {
    ...
    return (
        <div className="space-y-16">
            {/* 🔹 Hero Section */}
            <Hero image={'/images/c837a6_64dde2e4f4924a159161489c70cc7e79~mv2.avif'} titre={"NOUVEAUTÉS"} />
