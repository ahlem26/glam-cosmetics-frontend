...
export default function Visage() {
    ...
    const produitsVisage = useMemo(() => {
        return produits.filter((produit) => produit.categorie === "Visage");
    }, []);
    ...
    const { minPrix, maxPrix } = useMemo(() => {
        const allPrices = produitsVisage.map(p => p.promo ? p.promoPrix : p.prix);
        return {
    ...
    const produitsFiltres = produitsVisage.filter(p => {
    ...
    return (
        <div className="space-y-16">
            {/* 🔹 Hero Section */}
            <Hero image={'/images/c837a6_c30482248b534960807a2e70dba8b7c8~mv2.avif'} titre={"VISAGE"} />
