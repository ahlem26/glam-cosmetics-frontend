...
const categories = [
    "Tout", "Visage", "Accessoires", "Parfums", "Cheveux", "Maquillage", "Corps", "Nouveautés", "Meilleures ventes"
];

export default function Promos() {
    // 🔹 On filtre les produits de la catégorie "Visage"
    const produitsPromos = useMemo(() => {
        return produits.filter(p => p.promo);
    }, []);
    ...
    const { minPrix, maxPrix } = useMemo(() => {
        const allPrices = produitsPromos.map(p => p.promo ? p.promoPrix : p.prix);
        return {
    ...
    const produitsFiltres = produitsPromos.filter(p => {
    ...
    return (
        <div className="space-y-16">
            {/* 🔹 Hero Section */}
            <Hero image={'/images/c837a6_e8fa5af76e794202be430fbf0d0c3f56~mv2.avif'} titre={"PROMOS"} />
